...
async def upload_codebase(file: UploadFile = File(...)):
    ...
    zip_path = f"{UPLOAD_DIR}/{repo_id}.zip"
    repo_path = f"{REPO_DIR}/{repo_id}"
    ...
    store_codebase(repo_path,repo_id)

    return {"message": "Indexed successfully", "session_id": repo_id}
...
def ask_code_assistant(req: Query):
    search_Results= retrieve_chunks(query= req.question, repo_id= req.repo_id)
    context = context_structure(search_Results, repo_id= req.repo_id)
